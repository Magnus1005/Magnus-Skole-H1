...
namespace Plukliste
{
    public class BLL
    {
        ...
        public BLL(string importPath = "import", string exportPath = "export", string letterPath = "print", string letterTemplatePath = "letterTemplates")
        {
            ...
            if (!System.IO.Directory.Exists(letterPath))
            {
                System.IO.Directory.CreateDirectory(letterPath);
            }
            this.letterTemplatePath = letterTemplatePath;
            if (!System.IO.Directory.Exists(letterTemplatePath))
            {
                System.IO.Directory.CreateDirectory(letterTemplatePath);
            }
            this.index = 0;
        ...
        public Pluklist getFile(string v)
        {
        ...
        public Pluklist readFile(string filename)
        {
            ...
            statusData.filePath = filename;
            statusData.fileName = Path.GetFileName(filename);
        ...
        public string moveFile()
        {
            ...
            File.Move(files[index], $"{importPath}\\{filewithoutPath}");
